...
当用户要求调整动画顺序、效果、时长或具体对象出现方式时，运行独立 [`customize-animations`](../../skills/deepppt/workflows/customize-animations.md) 工作流。

```bash
# 从真实顶层 <g id> 锚点生成可编辑模板
python3 skills/deepppt/scripts/animation_config.py scaffold <project>

# 导出前校验引用是否存在
python3 skills/deepppt/scripts/animation_config.py validate <project>

# 导出时会自动读取 <project>/animations.json
python3 skills/deepppt/scripts/svg_to_pptx.py <project>
```
...
```bash
# 换效果
python3 skills/deepppt/scripts/svg_to_pptx.py <project> -t push --transition-duration 0.6

# 关闭转场
python3 skills/deepppt/scripts/svg_to_pptx.py <project> -t none

# 每 5 秒自动翻页（展厅 / 自动循环）
python3 skills/deepppt/scripts/svg_to_pptx.py <project> --auto-advance 5
```
...
# 关闭页内动画
python3 skills/deepppt/scripts/svg_to_pptx.py <project> -a none

# 改用单一效果（仍走默认的 after-previous 自动级联）
python3 skills/deepppt/scripts/svg_to_pptx.py <project> --animation fade

# 改为单击触发（演讲者控制节奏）
python3 skills/deepppt/scripts/svg_to_pptx.py <project> --animation-trigger on-click

# 自定义节奏
python3 skills/deepppt/scripts/svg_to_pptx.py <project> --animation mixed \
        --animation-stagger 0.6 --animation-duration 0.5

# 所有组进入页面时同时入场
python3 skills/deepppt/scripts/svg_to_pptx.py <project> --animation-trigger with-previous
```
...
无论是否打算开启动画，Executor 都应该把逻辑分块包进 `<g id>`。`skills/deepppt/references/shared-standards.md` 已将这一点列为强制要求。
...
完整 `svg_to_pptx.py` 参考：[`scripts/docs/svg-pipeline.md`](../../skills/deepppt/scripts/docs/svg-pipeline.md)。
