...
> "用这个模板做：`skills/deepppt/templates/layouts/academic_defense/`" ✅
> "用上次那个模板：`projects/last_deck/template/`" ✅
> "做一份产品介绍，模板用 `/Users/me/Desktop/our_brand_v3/`" ✅

AI 会把这个目录里的 SVG、`design_spec.md` 和素材复制到项目目录，然后进入 Strategist 阶段。路径可以是任意位置——内置库的 `skills/deepppt/templates/layouts/` 下、上一个项目的 `template/` 文件夹、或者磁盘上其他任何地方都行。
...
- [`templates/brands/README.md`](../../skills/deepppt/templates/brands/README.md) — 仅身份预设（color / typography / logo / voice / icon style），无 SVG 页面；Anthropic、Google
- [`templates/layouts/README.md`](../../skills/deepppt/templates/layouts/README.md) — 仅结构样板（canvas / page structure / page types / SVG roster），无身份；academic_defense、government_blue/red、ai_ops、medical_university、pixel_retro、psychology_attachment
- [`templates/decks/README.md`](../../skills/deepppt/templates/decks/README.md) — 完整 PPT 复刻（身份 + 结构 + 中间段）；招商银行、中国电建_*、中汽研_*、重庆大学、中国电信
...
完整规范见 [`workflows/create-template.md`](../../skills/deepppt/workflows/create-template.md)。本节是面向用户的简要版本——你只需要在 IDE 对话里说：
...
**强烈推荐：直接给原始 `.pptx` 文件。** 当前的 PPTX 导入管线已经做到接近高保真还原——工作流会用 [`pptx_template_import.py`](../../skills/deepppt/scripts/pptx_template_import.py) 直接读取 OOXML，提取主题色、字体、每个 master 的主题摘要、母版/版式结构、placeholder 元数据和可复用图片资源。它会输出作为机器事实源的 layered `svg/`，以及用于视觉预览的自包含 `svg-flat/`，再交给 Template_Designer 重建出干净可维护的 SVG。封面、章节、装饰繁复的页面都能稳定还原，这是目前最靠谱的派生路径。
...
1. 跑 [`svg_quality_checker.py`](../../skills/deepppt/scripts/svg_quality_checker.py) 验证（硬门，不通过不入库）
2. 把模板 ID 注册到 [`layouts_index.json`](../../skills/deepppt/templates/layouts/layouts_index.json)
3. 同步 [`templates/layouts/README.md`](../../skills/deepppt/templates/layouts/README.md) 表格

注册让模板**可被发现**——下次有人问"有哪些模板可用？"时，AI 会从索引里把它列出来。要在新项目里用它，仍然按 SKILL.md Step 3 的规则：在第一条消息里把目录路径写出来，例如 `用这个模板：skills/deepppt/templates/layouts/<your_template_id>/`。
...
- **派生新模板** = 入全局库，在 `skills/deepppt/templates/layouts/` 下，未来所有项目都能调用
...
- [`workflows/create-template.md`](../../skills/deepppt/workflows/create-template.md) — 完整工作流规范（面向 AI 执行）
- [`templates/layouts/README.md`](../../skills/deepppt/templates/layouts/README.md) — 现有模板一览
- [`references/template-designer.md`](../../skills/deepppt/references/template-designer.md) — 模板设计师角色定义和 SVG 技术约束
