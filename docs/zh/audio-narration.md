...
完整流程见 [`workflows/generate-audio.md`](../../skills/deepppt/workflows/generate-audio.md)。
...
已写入 `skills/deepppt/requirements.txt`。`edge-tts` 调用微软的在线 TTS 服务，**生成时**需要联网；生成后的音频是本地文件，PowerPoint 播放和视频导出都不依赖网络。云端 TTS provider 不需要额外 Python 包，直接通过 HTTPS 调用；按 `.env.example` 配置对应 API Key 即可。
