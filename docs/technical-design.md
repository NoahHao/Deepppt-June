...
PPT Master is not PPT-only — the same SVG → DrawingML pipeline produces square posters, 9:16 stories, A4 prints. Format-specific conventions (ratios, safe zones, brand areas) live in [`references/canvas-formats.md`](../skills/deepppt/references/canvas-formats.md).
...
The catalog of *how an image is placed on a slide* (full vocabulary in [`references/image-layout-patterns.md`](../skills/deepppt/references/image-layout-patterns.md)) splits 72 numbered techniques into two layers that compose freely:
...
This is the key insight that's easy to miss when reading the code. The same modules under `skills/deepppt/scripts/svg_finalize/` are used in two places, for two different products.
