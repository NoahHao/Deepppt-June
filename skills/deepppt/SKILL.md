...
description: >
  ...
  multi-role collaboration. Also includes /kb-search for PPT knowledge base retrieval.
  Use when user asks to "create PPT", "make presentation", "生成PPT", "做PPT",
  "制作演示文稿", mentions "deepppt", or runs /kb-search for PPT material discovery.
---
...
>
> - `deepppt` is a repository-specific workflow, not a general application scaffold
...
| `${SKILL_DIR}/scripts/source_to_md/pdf_splitter.py` | PDF Smart Splitter — auto-chunk long PDFs by chapter |
...
| `kb-search` | `workflows/kb-search.md` | KB knowledge base search — search local folders + web for PPT materials; slide-level content indexing with extract_slides recall |
...
| `split-pdf` | `workflows/split-pdf.md` | PDF Smart Splitter — chunk long PDFs (>10 pages) into context-safe units preserving chapter/section integrity. Triggers on `/split-pdf` or when user asks to "拆分PDF", "切割长文档". Auto-detects model context window, splits at chapter boundaries, supports concurrent chunk processing. |
| `multilingual-audio-gen` | `workflows/multilingual-audio-gen.md` | Multilingual Audio-Video Generation — generate speaker notes + TTS audio narration in any language, export video-ready PPTX with guaranteed audio-before-advance timings. Triggers on `/Audio Gen`, "生成讲解词及语音", "多语言配音". Uses `scripts/multilingual_audio_video/orchestrator.py` for one-command pipeline. Lazy-loaded — only activates on explicit user invocation. |
| `style-convert` | `workflows/style-convert.md` | PPTX Style & Language Conversion — extract Chinese text → AI translation → apply back with auto font-resize. Also supports white↔black background conversion. Triggers on `/style_convert`, "翻译PPT", "风格转换". Uses `scripts/style_convert/orchestrator.py`. Zero external dependencies (stdlib only). |

---

## Workflow

### Step 0: PDF Length Check (NEW — auto-triggered)

🚧 **GATE**: The user has provided a PDF source file.

> **Purpose**: Prevent information loss when processing long PDFs (>10 pages). Automatically split the PDF into context-safe chunks before conversion, preserving chapter/section integrity.

**When to trigger:**
- User provides a PDF file as source
- The PDF has more pages than the model can safely process in one pass

**Skip when:**
- Source is NOT a PDF (DOCX, URL, text, etc.)
- PDF ≤ 10 pages (proceed directly to Step 1)

**Auto-detection:**
```bash
python3 ${SKILL_DIR}/scripts/source_to_md/pdf_splitter.py <pdf_file> --analyze --json
```

If `total_pages > safe_pages_per_chunk`, the splitter will be invoked automatically:

```bash
python3 ${SKILL_DIR}/scripts/source_to_md/pdf_splitter.py <pdf_file> \
  --max-pages <safe_pages> \
  --output-dir <project_parent>/_pdf_chunks/
```

**After splitting**, each chunk in `split_manifest.json` is processed as an independent Step 1–7 pipeline run. This enables:
- **Sequential processing** — chunks processed one-by-one in a single session
- **Parallel processing** — multiple model instances process different chunks concurrently (see `workflows/split-pdf.md` §3b)

> ⚠️ **Context budget**: the splitter automatically accounts for skill code (~25K), script context (~8K), system prompts (~12K), and per-chunk processing overhead (~15K) when computing the safe page limit. The formula errs conservative — "宁可少，不能多".

**✅ Checkpoint — PDF checked; chunks created if needed. Proceed to Step 1 for the first chunk.**
...
|---------------|---------|
| PDF file | `python3 ${SKILL_DIR}/scripts/source_to_md/pdf_to_md.py <file> --render-vector-figures` |
...
>
> > **PDF vector figures (MANDATORY)**:
> > `pdf_to_md.py` MUST always run with `--render-vector-figures`. Many PDFs store
> > charts, diagrams, and illustrations as vector drawing commands (not raster images),
> > which the default `type==1` image block scan cannot capture. Without this flag,
> > all vector-based figures in the PDF are silently lost. Chinese captions ("图1-1")
> > are also supported via the updated `FIGURE_CAPTION_RE` regex.
...
>
> ⚠️ **Image source priority (MANDATORY)**: 原材料中的图片优先使用，AI生成仅作补充。
> 1. 源文档转换时自动提取的图片 (PDF→PNG/JPEG, DOCX/PPTX→EMF/WMF/PNG) — **优先使用**
> 2. 用户手动放入 `images/` 的图片 — **其次**
> 3. AI 生成 (ImageGen) — **仅当前两者都不可用时才触发**
>
> 在 Eight Confirmations 第8项中，AI必须首先报告源文档提取到的图片数量和使用计划，
> 然后才考虑是否需要补充AI生成图片。不要默认跳过原材料图片直接AI生成！
>
> ⚠️ **RGBA透明通道修复 (MANDATORY)**: PDF提取的PNG图片常含RGBA透明通道。
> SVG/PPTX渲染时透明部分会被显示为黑色。必须在base64编码前将RGBA转RGB（白底填充）。
> 使用 Pillow: `bg = Image.new('RGB', img.size, (255,255,255)); bg.paste(img, mask=img.split()[3])`。
...
**Live Preview Auto-Startup (Mandatory)**: before the first SVG, automatically start the browser editor in **share mode** (accessible from LAN for team collaboration) and keep it running continuously through Executor + Step 7 export:

```bash
# Auto-generate token and start share server
python3 ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --live --share --token $(python3 -c "import secrets,string;print(''.join(secrets.choice(string.ascii_letters+string.digits) for _ in range(12)))") --host 0.0.0.0 --timeout 0
```
- Start it immediately when Executor begins; `svg_output/` may be empty.
- **Must output the share URL + local review panel URL** for the user:
  ```
  Share (for team):  http://<local-ip>:5050/?token=<token>
  Review (publisher): http://localhost:5050/review
  ```
- Team members can browse slides and add annotations via the share link.
- Annotations go into a **pending queue**; publisher reviews at `/review` and approves/rejects.
- Approved annotations are applied to SVG files via `set_annotation()`.
- Port conflict → `--port <other>` and report the actual URL.
- `--timeout 0` disables idle timeout so the server stays alive for extended review sessions.
