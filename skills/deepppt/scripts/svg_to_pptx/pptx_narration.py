...
def probe_audio_duration(audio_path: Path) -> float | None:
    """Return duration in seconds. Tries ffprobe → mutagen → Xing header."""
    ...
    # 2. mutagen (accurate, handles VBR)
    try:
    ...
    # 3. Last resort: Xing/VBR header → file-size estimate
    try:
        with open(audio_path, 'rb') as f:
            ...
            # Try to find Xing header for accurate frame count
            import struct as _struct
            # Scan for Xing/Info header (after first frame)
            for pos in range(len(data) - 128):
                if data[pos:pos+4] in (b'Xing', b'Info'):
                    flags = _struct.unpack_from('>I', data, pos+4)[0]
                    offset = pos + 8
                    if flags & 0x01:  # frame count present
                        frame_count = _struct.unpack_from('>I', data, offset)[0]
                        offset += 4
                    else:
                        frame_count = 0
                    if flags & 0x02:  # byte count present
                        byte_count = _struct.unpack_from('>I', data, offset)[0]
                        size = byte_count  # use header's byte count
                    break

            # Estimate from first frame's bitrate
            bitrate = 128000
            for i in range(len(data) - 4):
                if data[i] == 0xFF and (data[i + 1] & 0xE0) == 0xE0:
                    bitrate_table = [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0]
                    bitrate_idx = (data[i + 2] >> 4) & 15
                    if 0 < bitrate_idx < 15:
