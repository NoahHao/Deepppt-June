...
Configuration source (process env wins, `.env` is the fallback layer):
  ...
  2. The first `.env` found among:
     ...
     - Skill directory (e.g. `~/.agents/skills/deepppt/.env`)
     - Repo root (when running from a clone)
     - `~/.deepppt/.env` (user-level config)
...
BACKEND_REGISTRY = {
    ...
    "qwen36-svg": {
        "module": "backend_qwen36_svg",
        "tier": "experimental",
        "label": "Qwen3.6 SVG -> PNG (vLLM Chat)",
        "default_model": "qwen3.6-35b",
        "key_hint": "QWEN_API_KEY",
        "aliases": ["qwen36_svg", "qwen36svg", "qwen-svg", "local"],
    },
}
