...
- High-quality cloud mode: provider API key is set before use:
  ...
  - Keys may live in the current process environment or the first `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/skills/deepppt/.env`), clone repo root, `~/.deepppt/.env`
...
```bash
python3 skills/deepppt/scripts/notes_to_audio.py --list-voices --locale <locale>
```
...
```bash
python3 skills/deepppt/scripts/notes_to_audio.py --provider minimax --list-voices
python3 skills/deepppt/scripts/notes_to_audio.py --provider qwen --list-voices
python3 skills/deepppt/scripts/notes_to_audio.py --provider cosyvoice --list-voices
```
...
- (If skipped embedding) one-line hint on how to embed later: `python3 skills/deepppt/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.
